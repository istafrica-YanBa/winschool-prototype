...
const exportHistory = async (format = 'csv') => {
  try {
    const historyData = {
      title: selectedBook.value?.title || 'Unknown Book',
      author: selectedBook.value?.author || 'Unknown Author',
      isbn: selectedBook.value?.isbn || 'Unknown ISBN',
      entries: readingHistory.value
    }
    
    addToast({
      message: `Exporting reading history as ${format.toUpperCase()}...`,
      type: 'info'
    })
    ...
    addToast({
      message: 'Failed to export reading history',
      type: 'error'
    })
  }
}
